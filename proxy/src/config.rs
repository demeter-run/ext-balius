...
impl Config {
    pub fn new() -> Self {
        Self {
            ...
            balius_port: env::var("BALIUS_PORT")
                .expect("BALIUS_PORT must be set")
                .parse()
                .expect("BALIUS_PORT must a number"),
            balius_dns: env::var("BALIUS_DNS").expect("BALIUS_DNS must be set"),
