...
use crate::{get_config, BaliusWorker, Error, State};
...
impl Default for Metrics {
    fn default() -> Self {
        ...
        let reconcile_failures = IntCounterVec::new(
            opts!(
                "balius_operator_crd_reconciliation_errors_total",
        ...
        let metrics_failures = IntCounterVec::new(
            opts!(
                "balius_operator_metrics_errors_total",
...
impl Metrics {
    ...
    pub fn reconcile_failure(&self, crd: &BaliusWorker, e: &Error) {
...
pub fn run_metrics_collector(state: Arc<State>) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            let query = format!(
                "sum by (consumer, network, tier) (increase(balius_proxy_http_total_request{{status_code!~\"401|429|503\"}}[{start}s] @ {}))",
